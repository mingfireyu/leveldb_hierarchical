#!/bin/bash
#totalsize=$(expr 1024 \* 1024 \* 1024 \* 1)
...
time out-shared/db_bench --db="$db" --benchmarks="$benchmarks" --value_size="$value_size" --num="$num" --bloom_bits="$bloom_bits" --log_open="$log_open" --max_file_size=4194304 > bench_result_"$value_size"B_"$num"_l"$log_open" 
